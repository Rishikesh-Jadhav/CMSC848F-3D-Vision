...
def save_plot(thresholds, avg_f1_score, args):
    ...
    f1_score_plots_folder = 'f1_score_plots'
    plt.savefig(os.path.join(f1_score_plots_folder, f'eval_{args.type}.png'), bbox_inches='tight')
...
def save_rgb_images(feed_dict,step):
        image=feed_dict['images']
        image=image.squeeze(0).cpu().numpy()
        image = cv2.resize(image, (256, 256))
        filename = os.path.join(rgb_images, f"{step}_img.jpg")
...
#------------------------------------------------------------------------------------------------------------------------------
        ...
        if img_step == 0:
            if args.type == "vox":      
                render_voxel(predictions[0], output_path=f'images_voxel/q_2-1-pred-{num}.gif')
                vox_gt = feed_dict['voxels'].to(args.device)
                render_voxel(vox_gt[0], output_path=f'images_voxel/q_2-1-gt-{num}.gif')
            if args.type == "point":
                # save_rgb_images(feed_dict, num)
                render_point_cloud(predictions, output_path=f'images_point/q_2-2-pred-{num}.gif')
                gt_points = sample_points_from_meshes(mesh_gt, args.n_points)
                render_point_cloud(gt_points, output_path=f'images_point/q_2-2-gt-{num}.gif')
            if args.type == "mesh":
                render_mesh(predictions, output_path=f'images_mesh/q_2-3-pred-{num}.gif')
                render_mesh(mesh_gt.to(args.device), output_path=f'images_mesh/q_2-3-gt-{num}.gif')
