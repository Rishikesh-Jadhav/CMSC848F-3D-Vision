...
class VoxelDecoder(nn.Module):
    def __init__(self):
        ...
        self.decoder_blocks = nn.ModuleList([
            ...
            self.decoder_block(16, 8),
            self.decoder_block(8, 4, last_layer=True)
        ])

        self.final_layer = nn.Sequential(
            nn.ConvTranspose3d(4, 1, kernel_size=1),
            nn.BatchNorm3d(1)
        )

    def decoder_block(self, in_channels, out_channels, last_layer=False):
        layers = []
...
class PointDecoder(nn.Module):
    def __init__(self, point_size):
        ...
        self.layer = nn.Sequential(
            nn.Linear(512, 512),  # mlp_0
            nn.ReLU(),
            nn.Linear(512, 1024),  # mlp_1
            nn.ReLU(),
            nn.Linear(1024, 2048),  # mlp_2
            nn.ReLU(),
            nn.Linear(2048, 2048),  # mlp_3
            nn.ReLU(),
            nn.Linear(2048, 2048),  # mlp_4
            nn.ReLU(),
            nn.Linear(2048, self.point_size * 3)  # mlp_5
        )

    def forward(self, feats):
        points = self.layer(feats)
        points = points.view(-1, self.point_size, 3)  # Reshape to the desired point cloud format
...
class MeshDecoder(nn.Module):
    def __init__(self, vert_size):
        ...
        self.layer = nn.Sequential(
            ...
            nn.Linear(2048, 2048),  # mlp_4
            nn.ReLU(),
            nn.Linear(2048, self.vert_size * 3)  # mlp_5
        )
