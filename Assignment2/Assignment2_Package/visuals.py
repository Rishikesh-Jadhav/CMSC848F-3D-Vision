...
def render_voxel(vox, output_path):
    ...
    render_360_degree_mesh(mesh, output_path=output_path, device=device)


######################### POINT CLOUD VISUALIZATION ###################
# get_points_renderer
def create_points_renderer(image_size=512, radius=0.01, background_color=(1, 1, 1)):
...
# render_360_pc
def render_360_degree_point_cloud(point_cloud, image_size=256, output_path='images/q_5-1_pc1.gif', device=None):
    renderer = create_points_renderer(image_size=image_size)
...
def render_point_cloud(points, output_path):
    device=points.device
    points = points.detach()[0]
    color = (points - points.min()) / (points.max() - points.min())
    pc = pytorch3d.structures.Pointclouds(points=[points], features=[color]).to(device)
    render_360_degree_point_cloud(pc, output_path=output_path, device=device)

######################### MESH VISUALIZATION #######################
# get_mesh_renderer
def create_mesh_renderer(image_size=512, lights=None, device=None):
    if device is None:
    ...
    return renderer
# render_360_mesh
def render_360_degree_mesh(mesh, image_size=256, output_path='images/q_1-1.gif', device=None, dist=3):
    renderer = create_mesh_renderer(image_size=image_size)
    num_views=12
    angles = np.linspace(-180, 180, num_views, endpoint=False)
    images = []
    for i in range(num_views):
...
def render_mesh(mesh, output_path):
    ...
    textures = textures * torch.tensor([0.7, 0.7, 1], device = device)
    mesh.textures=pytorch3d.renderer.TexturesVertex(textures)
    render_360_degree_mesh(mesh.detach(), output_path=output_path, device=device, dist=1.5)
