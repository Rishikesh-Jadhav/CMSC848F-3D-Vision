...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        # Creating an MLP for spatial coordinates with optional skip connections
        ...
        # Seting whether the model is view-dependent or not
        self.view_dep = view_dep

        if self.view_dep: # If view-dependent, create a linear layer for transforming spatial features
            self.linear = torch.nn.Linear(cfg.n_hidden_neurons_xyz, cfg.n_hidden_neurons_xyz+1)

            # Create an MLP for view-dependent features with optional skip connections
            ...
            # Create a linear layer for feature output
            self.feature_linear = torch.nn.Linear(cfg.n_hidden_neurons_dir, 3)
        else:
            # If view-independent, create a linear layer for direct output
            self.linear = torch.nn.Linear(cfg.n_hidden_neurons_xyz, 4)

    def forward(self, ray_bundle):
        # Compute the harmonic embedding for spatial coordinates
        embedding_xyz = self.harmonic_embedding_xyz(ray_bundle.sample_points.view(-1, 3))

        # Pass the embedding through the spatial MLP
        y = self.mlp_skip(x=embedding_xyz, z=embedding_xyz)            
        
        # If view-dependent, apply the linear transformation to 'y'
        if self.view_dep:
            y = self.linear(y)

            # Compute the harmonic embedding for view directions and concatenate with 'y'
